...
public class Main {
    ...
    public static void main(String[] args) throws InterruptedException {
        ...
        Thread thread1 = new Thread(() -> {
            for (String text : texts) {
                if (isThread1(text) && !isThread2(text)) {
        ...
        Thread thread3 = new Thread(() -> {
            for (String text : texts) {
                if (!isThread1(text) && isThread3(text)) {
                    incrementCounter(text.length());
                }
            }
        });
    ...
    public static boolean isThread1(String text) {
        return text.equals(new StringBuilder(text).reverse().toString());
    }

    public static boolean isThread2(String text) {
    ...
    public static boolean isThread3(String text) {
